...
> >
> > 1. {% tool [Sub-sample sequences files ](toolshed.g2.bx.psu.edu/repos/peterjc/sample_seqs/sample_seqs/0.2.6) %} with the following parameters:
...
Sequence quality control is therefore an essential first step in your analysis. We will use two popular tools for evaluating the quality of our raw reads: __Falco__ and __MultiQC__.

> <comment-title></comment-title>
> In order to visualize the data from both collections together in the __MultiQC__ tool, it will be necessary to combine the results generated by __Falco__.
...
> 2. Rename the outputs as `Falco unprocessed control miRNA: RawData` and `Falco unprocessed control miRNA: Webpage`
...
>      - *"1.Input Collections"*: `Falco unprocessed control miRNA: RawData`
>      - *"2.Input Collections"*: `Falco unprocessed BR treated mRiNA: RawData`
> 5. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy3) %} with the following parameters:
>    - In *"Results"*:
>      - *"Which tool was used generate logs?"*: `FastQC`
>        - In *"FastQC output"*:
>          - {% icon param-repeat %} *"Insert FastQC output"*
>            - {% icon param-collection %} *"FastQC output"*: select the output of __Merge Collection__ tool
>    - In *"Report title"*: `miRNA initial quality check`
> 6. Click on the {% icon galaxy-eye %} (eye) icon and inspect the generated HTML file
>
{: .hands_on}
...
To remove the adapters contamination, we will employ the __Trim Galore__ tool, a wrapper script around [__Cutadapt__](https://journal.embnet.org/index.php/embnetjournal/article/view/200) and __Falco__ which automates quality and adapter trimming.
...
> 2. Rename the outputs as `Falco processed control miRNA: RawData` and `Falco processed control miRNA: Webpage`
> 3. Repeat the previous steps with the `BR treated miRNA trimmed` collection.
> 4. {% tool [Merge Collections](__MERGE_COLLECTION__) %} with the following parameters:
>    - In *"Input collections"*:
>        - *"1.Input Collections"*: `Falco processed control miRNA: RawData`
>        - *"2.Input Collections"*: `Falco processed BR treated miRNA: RawData`
> 5. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy3) %} with the following parameters:
>    - In *"Results"*:
>      - *"Which tool was used generate logs?"*: `FastQC`
>        - In *"FastQC output"*:
>           - {% icon param-repeat %} *"Insert FastQC output"*
>               - {% icon param-collection %} *"FastQC output"*: select the output of __Merge Collection__ tool
>    - *"Report title"*: `miRNA trimmed quality check`
> 6. Click on the {% icon galaxy-eye %} (eye) icon and inspect the generated HTML file
>
{: .hands_on}
...
>
> 1. {% tool [MiRDeep2 Mapper](toolshed.g2.bx.psu.edu/repos/rnateam/mirdeep2_mapper/rbc_mirdeep2_mapper/2.0.0.8.1) %} with the following parameters:
...
> 2. Rename the outputs as `Falco unprocessed control mRNA: RawData` and `Falco unprocessed control mRNA: Webpage`
> 3. Repeat the previous step with the `BR treated mRNA` collection.
> 4. {% tool [Merge Collections](__MERGE_COLLECTION__) %} with the following parameters:
>    - In *"Input collections"*:
>        - *"1.Input Collections"*: `Falco unprocessed control mRNA: RawData`
>        - *"2.Input Collections"*: `Falco unprocessed BR treated mRNA: RawData`
> 5. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy3) %} with the following parameters:
>    - In *"Results"*:
>      - *"Which tool was used generate logs?"*: `FastQC`
>        - In *"FastQC output"*:
...
>
> 1. {% tool [Salmon quant](toolshed.g2.bx.psu.edu/repos/bgruening/salmon/salmon/1.10.1+galaxy2) %} with the following parameters:
